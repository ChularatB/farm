...
export default function Profile() {
   ...
   const handleSizeChange = (e) => {
      // เอาเฉพาะตัวเลข 0-9 เท่านั้น (ลบตัวอักษรอื่นทิ้งเกลี้ยง)
      const onlyNums = e.target.value.replace(/[^0-9]/g, '');
      setFarmInfo({ ...farmInfo, size: onlyNums });
   };

   const handleDevicesChange = (e) => {
      // เอาเฉพาะตัวเลข 0-9 เท่านั้น
      const onlyNums = e.target.value.replace(/[^0-9]/g, '');
      setFarmInfo({ ...farmInfo, devices: onlyNums });
   };
   ...
   const handleSaveFarmInfo = async () => {
      ...
      try {
         const res = await fetch('/api/farm/update', {
            ...
            body: JSON.stringify({
               // 🛑 ใช้ parseInt เพื่อแปลงข้อความให้กลายเป็นตัวเลข หรือเป็น 0 ถ้าไม่ได้กรอก
               farm_size: parseInt(farmInfo.size) || 0,
               total_devices: parseInt(farmInfo.devices) || 0
            })
         });
         ...
         <div className="grid grid-cols-2 gap-4 mb-6">
            ...
            <div className="bg-white p-4 rounded-3xl shadow-sm text-center relative border border-gray-100">
               ...
               {loadingInfo ? (
                  ...
                  <input
                     type="text" // 🛑 เปลี่ยนเป็น text ก่อน เพื่อให้ Regex ดักจับได้สมูทๆ
                     inputMode="numeric" // 🛑 บังคับให้คีย์บอร์ดมือถือเด้งแป้นตัวเลขขึ้นมา
                     value={farmInfo.size}
                     onChange={handleSizeChange} // 🛑 เรียกใช้ฟังก์ชันใหม่
            ...
            <div className="bg-white p-4 rounded-3xl shadow-sm text-center border border-gray-100">
               ...
               {loadingInfo ? (
                  ...
                  <input
                     ...
                     onChange={handleDevicesChange} // 🛑 เรียกใช้ฟังก์ชันใหม่
                     className="w-full text-center border-b border-primary-medium focus:outline-none text-xl font-bold text-primary-dark"
                  />
               ) : (
